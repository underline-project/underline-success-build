Underline Core version *0.15.1* is now available from:

  <https://download.underline.org/underline-0.15.1/>
...
A number of changes to the way Underline Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 uline per kB (0.00001 UCOIN). This is to prepare for dropping the minimum transaction fee to 0.1 uline per kB (0.0001 UCOIN) in the next release.
